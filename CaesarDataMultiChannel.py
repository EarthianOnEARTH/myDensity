...
class CaesarData(object):
    ...
    def __init__(self, detector="i03-c-cx1/dt/dtc-falconx8.1", dataname="channel03", experimentdir=""):
        ...
        # add some empty attributes for the spectra
        self.spectra=""
        ...
        self.geo_R = ""
        self.geo_deltaR = ""
        self.geo_chan_z = "" # this is the z position; in mm, of each channel wrt beam height
        # load default calibration to start with
        self.loadCalibrationFile('defaults')
        # call set geometry because we need it for the multi element angle calculation
        self.setGeometry()
    ...
    def updateSpectrum(self):
        ''' read, convert, plot '''
        self.__getSpectra()
        self.__convertSpectra()
        self.__showSpectra()

    def startLive(self, ctime=1):
       ''' set short exposure time, keep starting detector '''
       self.__DP.Stop()
       self.__DP.presetType = 'FIXED_REAL'
       self.__DP.presetValue = ctime
       loop = True
       print('Starting live, ctrl-c to stop')
       while loop:
           try:
               self.__DP.Snap()
               time.sleep(ctime)
               while self.__DP.state() == PyTango._PyTango.DevState.RUNNING:
                   time.sleep(0.2)
               time.sleep(2)
    ...
    def doTimeScan(self, npoints=5, timestep=5, scanname='timescan_'):
        ...
        self.__DP.Stop()
        self._CaesarData__DP.presetValue = timestep
        time.sleep(0.1)
        self._CaesarData__DP.presetType = 'FIXED_REAL'
        # do the timescan
        for ii in range(npoints):
            ...
            self.updateSpectrum()
            # save the data
            self.exportGSAS_EDX(myfilename="timestep_%d" % ii, mydirectory=datadir)
            # update stuff?
            pylab.draw()
            sys.stdout.flush()
    ...
    def doCaesarScan(self, scanname, checkpars=True, startfrompointN=0):
        ...
        # call getSpectrum to make sure spectrum is up to data, and recalculate energy
        self.__getSpectra()
        # prepare an array to put the data in - now angles x energy x channels
        self.caesar_image = np.zeros([len(self.caesar_tth_nominal), self.spectra.shape[1], self.nchan])
        self.caesar_image_original = ""
        # reset the mask
        self.maskCaesar()
        # apply the angle_calibration to the tth_angles
        ...
        self.caesar_realtime = np.zeros([self.caesar_tth_nominal.shape[0], self.nchan])
        self.caesar_livetime = np.zeros([self.caesar_tth_nominal.shape[0], self.nchan])

        # if we 'stop' the detector, we don't get a spectrum
        self.__DP.presetType = 'FIXED_REAL'
        ...
        try:
            ...
            for ii in range(startfrompointN, len(self.caesar_tth_nominal)):
                ...
                # set the count time
                self.__DP.presetValue = self.caesar_counttime[ii]
                ...
                # read the spectrum, livetime and the realtime - for all channels
                for jj,chan in enumerate(self.multicalib_chan):
                    self.caesar_realtime[ii, jj] = getattr(self.__DP, ('realtime%02d' % chan))
                    self.caesar_livetime[ii, jj] = getattr(self.__DP, ('livetime%02d' % chan))
                    spectrum = getattr(self.__DP, ('channel%02d' % chan))
                    # add to the image
                    self.caesar_image[ii, :, jj] = spectrum
        except:
            ...
            myscandata = {'caesar_image':self.caesar_image,
                        ...
                        'multicalib_A':self.multicalib_A,
                        'multicalib_E':self.multicalib_E,
                        'multicalib_chan':self.multicalib_chan,
                        'chan_z':self.geo_chan_z}
    ...
    def calculateCaesarScan(self):
        ...
        self.caesar_s1hg = np.ones(self.caesar_tth_nominal.shape) * s1hg_min
        self.caesar_w1 = np.ones(self.caesar_tth_nominal.shape) * w1_min
        self.caesar_w2 = np.ones(self.caesar_tth_nominal.shape) * w2_min
        # times
        counttime = np.ones(self.caesar_tth_nominal.shape) * counttimeB
        counttime[np.nonzero(self.caesar_tth_nominal<12)] = counttimeA
        counttime[np.nonzero(self.caesar_tth_nominal>20)] = counttimeC
        ...
        pylab.plot(self.caesar_tth_nominal, self.caesar_s1hg, '-.b', label='s1hg without boost')
        pylab.plot(self.caesar_tth_nominal, self.caesar_w1, '-.g', label='cs1hg without boost')
        pylab.plot(self.caesar_tth_nominal, self.caesar_w2, '-.r', label='cs2hg without boost')
        pylab.subplot(3,1,2)
        ...
        pylab.plot(self.caesar_tth_nominal, self.caesar_gaugelen, '-.b', label='without boost')
        pylab.subplot(3,1,3)
        pylab.title('nominal intensity')
        pylab.plot(self.caesar_tth_nominal, self.caesar_integral, '-.b', label='without boost')
        ...
        if boost:
            ...
            factor = 1. + ((self.caesar_tth_nominal - 10)*0.15) # first scan was 0.167
            ...
            pylab.subplot(3,1,1)
            pylab.title('slit gaps')
            pylab.plot(self.caesar_tth_nominal, self.caesar_s1hg, '-b', label='s1hg with boost')
            pylab.plot(self.caesar_tth_nominal, self.caesar_w1, '-g', label='cs1hg with boost')
            pylab.plot(self.caesar_tth_nominal, self.caesar_w2, '-r', label='cs2hg with boost')
            pylab.subplot(3,1,2)
            pylab.title('gauge length')
            pylab.plot(self.caesar_tth_nominal, self.caesar_gaugelen, '-b', label='with boost')
            pylab.subplot(3,1,3)
            pylab.title('nominal intensity')
            pylab.plot(self.caesar_tth_nominal, self.caesar_integral, '-b', label='with boost')
        else:
...
######  CALIBRATION FUNCTIONS #######
    ...
    def loadCalibrationFile(self, filename=None):
        if filename == 'defaults':
            print('load defaults from : /usr/Local/configFiles/PyBeamlineRoot/beamline/macros/FalconXcalib_default_calib.txt')
            filename = '/usr/Local/configFiles/PyBeamlineRoot/beamline/macros/FalconXcalib_default_calib.txt'
        elif filename == None:
            print('File should contain both angle and energy calibration for all channels')
            print('Channel, E0, E1, E2, A0, A1\n1, 0.0058079, 0.049974, 0, 0.0005, 0.0094\n2, etc, etc, ...')
            filename = input('Please give the full path to the calibration file [load default values] : ')
            if filename == '':
                print('load defaults from : /usr/Local/configFiles/PyBeamlineRoot/beamline/macros/FalconXcalib_default_calib.txt')
                filename = '/usr/Local/configFiles/PyBeamlineRoot/beamline/macros/FalconXcalib_default_calib.txt'
        calib = np.loadtxt(filename, skiprows=1, delimiter=',')
        self.multicalib_chan = calib[:, 0]
        self.nchan = len(self.multicalib_chan)
        self.multicalib_E = calib[:, 1:4]
        self.multicalib_A = calib[:, 4:6]
        if calib.shape[1]==7:
            self.geo_chan_z = calib[:, 6]
        print('Loading calib from : %s' % filename)
        print('_____________________')
        for ii in range(calib.shape[0]):
            print('channel %d : energy = %0.4f + %0.4f*channel + %0.4f * channel**2' % (self.multicalib_chan[ii], self.multicalib_E[ii, 0], self.multicalib_E[ii, 1], self.multicalib_E[ii, 2]))
        print('_____________________')
        for ii in range(calib.shape[0]):
            print('channel %d : angle offset = %0.4f*angle + %0.4f' % (self.multicalib_chan[ii], self.multicalib_A[ii, 0], self.multicalib_A[ii, 1]))
        print('_____________________')
        for ii in range(calib.shape[0]):
            print('channel %d : z position = %0.4f' % (self.multicalib_chan[ii],self.geo_chan_z[ii]))
        print('_____________________')
    ...
    def __applyEnergyCalibration(self):
        ...
        self.energy_unbinned = np.zeros((self.nchan, nchan_unbinned))
        for ii in range(self.nchan):
            self.energy_unbinned[ii,:] = self.multicalib_E[ii,0] + self.multicalib_E[ii,1]*channels + self.multicalib_E[ii,2]*channels**2
        # to avoid divide by zero error, fake the zero/negative values in energy
        self.energy_unbinned[np.nonzero(self.energy_unbinned<0.0000001)]=0.0000001
        self.energy = self.energy_unbinned.reshape((self.nchan, (old_div(nchan_unbinned,binning)), binning)).mean(2)
    ...
    def __applyAngleCalibration(self):
        ...
        Mod to consider N channels
        Include a calculation based on the detector geometry. The linear correction is applied _after_ this theoretical correction
        '''
        if type(self.caesar_tth_nominal)==np.ndarray:
            # N sets of caesar tth for N channels
            self.caesar_tth = np.zeros((self.nchan, len(self.caesar_tth_nominal)))
            for ii in range(self.nchan):
                # first calculate the theoretical true 2theta from the geometry for this channel
                dist = self.geo_R + self.geo_deltaR
                tth_theory = np.rad2deg(np.arccos(old_div(dist*np.cos(np.deg2rad(self.caesar_tth_nominal)), ((dist**2 + self.geo_chan_z[ii]**2)**0.5))))
                # then calculate the linear correction
                tth_offset = (self.caesar_tth_nominal*self.multicalib_A[ii, 0]) + self.multicalib_A[ii, 1]
                # final 2theta is the combination of these
                self.caesar_tth[ii, :] = tth_theory + tth_offset
        else:
        ...
        if type(self.spectrum_tth_nominal)==float:
            # first calculate the theoretical true 2theta from the geometry for this channel
            dist = self.geo_R + self.geo_deltaR
            tth_theory = np.rad2deg(np.arccos(old_div(dist*np.cos(np.deg2rad(self.spectrum_tth_nominal)), ((dist**2 + self.geo_chan_z**2)**0.5))))
            # spectrum_tth is now N values for N channels
            tth_offset = (self.spectrum_tth_nominal*self.multicalib_A[:, 0]) + self.multicalib_A[:, 1]
            self.spectrum_tth = tth_theory + tth_offset
        else:
    ...
    def showGeometry(self):
        ...
        print("detector slit 1 -> detector slit 2: %0.2f" % self.geo_deltaR)
    ...
    def setDetector(self, detector="", dataname=""):
        '''
            ...
            multichannel world - we will always get all channels. But might use this for display?
        '''
    ...
    def loadPickle(self, filename=None, binning=None):
        ...
        try:
            ...
            if type(tmp)==dict:
                ...
                if len([tmp['w1']])==1:
                    self.caesar_w1 = tmp['w1'] * np.ones(len(self.caesar_tth_nominal))
                else:
                    self.caesar_w1 = tmp['w1']
                if len([tmp['w2']])==1:
                    self.caesar_w2 = tmp['w2'] * np.ones(len(self.caesar_tth_nominal))
                else:
                    self.caesar_w2 = tmp['w2']
                if 'cs1vg' in tmp:
                    self.caesar_cs1vg = tmp['cs1vg']
                else:
                    self.caesar_cs1vg = '' # as in init, maybe not best choice
                if 'cs2vg' in tmp:
                    self.caesar_cs2vg = tmp['cs2vg']
                else:
                    self.caesar_cs2vg = '' # as in init, maybe not best choice
                ...
                # new pickles have the calibration
                if 'multicalib_A' in tmp:
                    print('Pickle contains calib data. Can use it, or keep the current calib')
                    loadcalib = input('load multichannel calibration data from pickle? y/[n] : ')
                    loadcalib = True if loadcalib.lower() in ['y', 'yes', 'true', '1'] else False
                    if loadcalib:
                        self.multicalib_A = tmp['multicalib_A']
                        self.multicalib_E = tmp['multicalib_E']
                        self.multicalib_chan = tmp['multicalib_chan']
                        # if the pickle has chan_z data, use it
                        if 'chan_z' in tmp:
                            self.geo_chan_z = tmp['chan_z']
                        print('loaded calib from pickle')
                    else:
                        print('not loading calib data, using current calib')
                # in either case, make sure the right calib is applied
                self.__applyEnergyCalibration()
                self.__applyAngleCalibration()
                # make an empty mask
                self.maskCaesar()
            ...
            ysize, xsize = self.caesar_image.shape[0], self.caesar_image.shape[1]
            if binning != 0:
                if np.mod(xsize, binning) == 0:
                    if len(self.caesar_image.shape)==2:
                    ...
                    elif len(self.caesar_image.shape)==3:
                        newim = np.zeros((ysize, (old_div(xsize,binning)), self.caesar_image.shape[2]))
                        self.energy = np.zeros((self.nchan, (old_div(xsize,binning))))
                        for chan in range(self.caesar_image.shape[2]):
                            newim[:,:,chan] = self.caesar_image[:,:,chan].reshape((ysize, (old_div(xsize,binning)), binning)).mean(2)
                            self.energy[chan, :] = self.energy_unbinned[chan, :].reshape(((old_div(xsize,binning)), binning)).mean(1)
                        self.caesar_image = newim
                else:
    ...
    def prenormaliseCaesar(self):
        ...
        if len(imE.shape)==2: # 2D, normal case
        ...
        elif len(imE.shape)==3: # 3D, multichannel case
            imE2 = np.zeros(imE.shape)
            imE3 = np.zeros(imE.shape)
            for chan in range(self.nchan):
                # first, simplest - counting times - included in live time
                live = np.tile(self.caesar_livetime[:,chan], (imE.shape[1], 1)).T
                imE2[:,:,chan] = old_div(imE[:,:,chan], live) # counts per second
            # Now the geometry corrections
            self.calculateGaugeVolumes()
            # normalise by the calculated integral
            integral = np.tile(self.caesar_integral, (imE.shape[1], 1)).T
            for chan in range(self.nchan):
                imE3[:,:,chan] = old_div(imE2[:,:,chan], integral)
        else:
            print('strange caesar_image')
    ...
    def correctEscapePeaks(self):
        '''Correct escape peaks using function derived in Clark_0416
            ...
            treat peak widths
            Need to do channel by channel'''
        # back up the current image
        self.caesar_image_original = self.caesar_image * 1.
        # correct channel by channel
        for ii in range(self.nchan):
            myenergy = self.energy[ii, :] * 1.
            imE = self.caesar_image[:,:,ii] * 1.
            escapef = 0.41501256 * np.exp(myenergy * -0.08536899)
            escapef = np.tile(escapef, (imE.shape[0], 1))
            ndx = np.nonzero(myenergy>20)[0][0]
            dE = myenergy[ndx+1] - myenergy[ndx] # channel width at 20 keV
    ...
    def calculateGaugeVolumes(self):
        ...
        # get the geometry
        # use the nominal angles here because we have multi channels with multi calibrations
        R = self.geo_R
        deltaR = self.geo_deltaR
        sample_d = self.geo_sample_diameter
        s1_distance = self.geo_s1_distance
        source_sigma = old_div((1/2.355) * (s1_distance * 0.900), (23500 - s1_distance))
        scanangles = self.caesar_tth_nominal
    ...
    def __getSpectra(self):
        ''' get the latest spectra from the detector '''
        if self.__DP!="":
            print("read the current spectra, correct Caesar angles")
            # energy_unbinned is the right size - nchan x length spectrum
            self.spectra = np.zeros((self.energy_unbinned.shape))
            # get the n spectra
            for ii,nn in enumerate(self.multicalib_chan):
                self.spectra[ii, :] = getattr(self.__DP, 'channel%02d' % nn)
            # get the nominal angle
...
##### DATA PROCESSING FUNCTIONS #########
    ...
    def maskCaesar(self, mask=None):
        ...
        if mask == None:
            ...
            self.maskE = np.ones((self.caesar_image.shape[0], self.caesar_image.shape[1]))
            ...
            tth_ndx_start = np.nonzero(self.caesar_tth_nominal>mask[0])[0][0]
            tth_ndx_end = np.nonzero(self.caesar_tt_nominalh<mask[1])[0][-1]+1
            meanenergy = self.energy.mean(0) # make one profile
            print('Using MEAN energy')
            energy_ndx_start = np.nonzero(meanenergy>mask[2])[0][0]
            energy_ndx_end = np.nonzero(meanenergy<mask[3])[0][-1]+1
    ...
    def __convertSpectra(self):
        ...
        if self.spectra != '':
            ...
            dsp = np.zeros(self.energy_unbinned.shape)
            for ii in range(self.nchan):
                # current data in dsp and Q
                dsp[ii, :] = 12.398 / (2*np.sin(theta[ii])*self.energy_unbinned[ii, :])
            Q = old_div(2*np.pi,dsp)
            # limits for interpolation
            dmax = 12.398 / (2*np.sin(theta.min())*10) # current angle, lowest energy
            dmin = 12.398 / (2*np.sin(theta.max())*self.energy_unbinned.max()) # current angle, highest energy
            ...
            # loop over N channels
            self.spectra_dspy = np.zeros((self.nchan, 4096))
            self.spectra_qy = np.zeros((self.nchan, 4096))
            self.spectra_dspx = dspgrid
            self.spectra_qx = Qgrid
            for ii in range(self.nchan):
                self.spectra_dspy[ii, :] = np.interp(dspgrid, dsp[ii, ::-1], self.spectra[ii, ::-1], 0, 0)
                self.spectra_qy[ii, :] = np.interp(Qgrid, Q[ii, :], self.spectra[ii, :], 0, 0)
        else:
            self.spectra_dspy = ""
            self.spectra_dspx = ""
            self.spectra_qy = ""
    ...
    def __convertImage(self):
        ''' convert data from energy to dspacing and Q '''
        # convert the spectrum, if available - do all channels
        if len(self.caesar_image) != 0:
            # determine the theta values
            theta = old_div(self.caesar_tth,2)
            # preallocate an output image
            image2 = np.zeros([self.caesar_image.shape[0], 4096, self.nchan])
            image3 = np.zeros([self.caesar_image.shape[0], 4096, self.nchan])
            dspprofile = np.zeros((self.nchan, 4096))
            Qprofile = np.zeros((self.nchan, 4096))
            # dspacing limits for the whole acquistion
            dmax = 12.398 / (2*np.sin(old_div(theta.min()*np.pi,180))*10) # lowest angle, lowest energy
            dmin = 12.398 / (2*np.sin(old_div(theta.max()*np.pi,180))*self.energy.max()) # highest angle, highest energy
            dspgrid = np.linspace(dmin, dmax, 4096)
            Qmin = old_div(2*np.pi,dmax)
            Qmax = old_div(2*np.pi,dmin)
            Qgrid = np.linspace(Qmin, Qmax, 4096)
            ...
            for chan in range(self.nchan):
                ...
                for ii in range(self.caesar_image.shape[0]):
                    # energy profile
                    prof = self.caesar_image[ii, :, chan]
                    prof = prof[::-1]
                    # as d spacing
                    dsp = 12.398 / (2*np.sin(old_div(theta[chan, ii]*np.pi,180))*self.energy[chan,:])
                    dsp = dsp[::-1] # increasing
                    image2[ii, :, chan] = np.interp(dspgrid, dsp, prof, 0, 0)
                    # as Q - flip profiles for increasing
                    Q = old_div(2*np.pi,dsp)
                    image3[ii, :, chan] = np.interp(Qgrid, Q[::-1], prof[::-1], 0, 0)
                # build the (naive) combined dsp profile - only do this if we haven't done something else
                dspprofile[chan, :] = image2[:,:,chan].sum(0)
                dspprofile_n = (image2[:,:,chan]!=0).sum(0)
                dspprofile[chan, :] = old_div(dspprofile[chan, :],dspprofile_n)
                dspprofile[chan, np.nonzero(np.isnan(dspprofile[chan, :]))] = 0 # remove nans
                print("correcting combined profile for number of datapoints")
                # build the (naive) combined Q profile
                Qprofile[chan, :] = image3[:,:,chan].sum(0)
                Qprofile_n = (image3[:,:,chan]!=0).sum(0)
                Qprofile[chan, :] = old_div(Qprofile[chan, :],Qprofile_n)
                Qprofile[chan, np.nonzero(np.isnan(Qprofile[chan, :]))] = 0 # remove nans
    ...
    def __convertImageBACKUP(self):
        ''' convert data from energy to dspacing and Q '''
        # convert the spectrum, if available - backup original
        if len(self.caesar_image) != 0:
                # determine the theta values
            theta = old_div(self.caesar_tth,2)
            # preallocate an output image
...
####### FIGURE HANDLING FUNCTIONS ############
    ...
    def __showSpectra(self, clear_fig=True):
        ...
        if self.spectra != '':
            ...
            pylab.subplot(2,1,1)
            for ii in range(self.nchan):
                pylab.plot(self.energy_unbinned[ii, :], self.spectra[ii, :], label='channel%02d, %0.3f deg nom.' % (self.multicalib_chan[ii], self.spectrum_tth_nominal))
            pylab.legend()
        ...
        if len(self.spectra_dspx) != 0:
            # lower figure - spectrum in dspacing
            pylab.subplot(2,1,2)
            if self.plotdsp:
                for ii in range(self.nchan):
                    pylab.plot(self.spectra_dspx, self.spectra_dspy[ii, :], label='Converted channel%02d, %0.3f deg' % (self.multicalib_chan[ii],self.spectrum_tth[ii]))
                pylab.legend()
                pylab.xlabel('d spacing / Angstroms')
                pylab.ylabel('intensity / AU')
            else:
                for ii in range(self.nchan):
                    pylab.plot(self.spectra_qx, self.spectra_qy[ii, :], label='Converted channel%02d, %0.3f deg' % (self.multicalib_chan[ii],self.spectrum_tth[ii]))
                pylab.legend()
    ...
    def __showImage(self):
        ...
        if len(self.caesar_image) != 0:
            ...
            if self.logscale==True:
                pylab.imshow(np.log(self.caesar_image[:,:,chan]*self.maskE), extent=[self.energy[chan,0], self.energy[chan,-1], self.caesar_tth[chan,-1], self.caesar_tth[chan,0]])
            else:
                pylab.imshow(self.caesar_image[:,:,chan]*self.maskE, extent=[self.energy[chan,0], self.energy[chan,-1], self.caesar_tth[chan,-1], self.caesar_tth[chan,0]])
            pylab.xlabel('energy / keV')
        ...
        if len(self.caesar_image_dsp) != 0:
            ...
            if self.plotdsp:
                if self.logscale==True:
                    pylab.imshow(np.log(self.caesar_image_dsp[:,:,chan]), extent=[self.caesar_image_dspx[0], self.caesar_image_dspx[-1], self.caesar_tth[chan,-1], self.caesar_tth[chan,0]])
                else:
                    pylab.imshow(self.caesar_image_dsp[:,:,chan], extent=[self.caesar_image_dspx[0], self.caesar_image_dspx[-1], self.caesar_tth[chan,-1], self.caesar_tth[chan,0]])
                pylab.xlabel('d spacing / Angstroms')
                pylab.ylabel('two theta / degrees')
            else:
                if self.logscale==True:
                    pylab.imshow(np.log(self.caesar_image_Q[:,:,chan]), extent=[self.caesar_image_Qx[0], self.caesar_image_Qx[-1], self.caesar_tth[chan,-1], self.caesar_tth[chan,0]])
                else:
                    pylab.imshow(self.caesar_image_Q[:,:,chan], extent=[self.caesar_image_Qx[0], self.caesar_image_Qx[-1], self.caesar_tth[chan,-1], self.caesar_tth[chan,0]])
                pylab.xlabel('Q / Angstroms-1')
                pylab.ylabel('two theta / degrees')
            pylab.axis("tight")
            ...
            pylab.subplot(3,1,3)
            if self.plotdsp:
                pylab.plot(self.caesar_image_dspx, self.caesar_image_dsp_profile[chan, :], label='Integrated profile')
                if self.caesar_image_dsp_profile_cor != "":
                    pylab.plot(self.caesar_image_dspx, self.caesar_image_dsp_profile_cor[chan, :], 'r', label='Corrected intensities')
                pylab.legend()
                pylab.xlabel('d spacing / Angstroms')
                pylab.ylabel('Intensity')
            else:
                pylab.plot(self.caesar_image_Qx, self.caesar_image_Q_profile[chan, :], label='Integrated profile')
                if self.caesar_image_Q_profile_cor != "":
                    pylab.plot(self.caesar_image_Qx, self.caesar_image_Q_profile_cor[chan, :], 'r', label='Corrected intensities')
                pylab.legend()
                pylab.xlabel('Q / Angstroms-1')
                pylab.ylabel('Intensity')
            pylab.axis("tight")
...
######## CAESAR DATA FUNCTIONS  ###########
    ...
    def exportGSAS_EDX(self, myfilename="psiche", mydirectory="/nfs/ruche-psiche/psiche-soleil/tempdata/com-psiche/test_directory"):
        '''Export current energy dispersive spectra as a series of gsas files,
            one for each channel. The x_y and pd format files are also saved'''
        for ii in range(self.nchan):
            chan = self.multicalib_chan[ii]
            self.__exportGSAS_EDX(channel=chan, filename=myfilename, directory=mydirectory)


    def __exportGSAS_EDX(self,channel=0, filename="psiche", directory="/nfs/ruche-psiche/psiche-soleil/tempdata/com-psiche/test_directory"):
        '''Export the current energy dispersive spectrum as a .gsas file
            Exports also a text file (.x_y) in two column format
            this is now a hidden __ function, which saves one channel'''

        # get the next filename - if a name is supplied or at SOLEIL (no tk)
        if (filename!="psiche") or SOLEIL:
            # use the given arguments
            myfilename = findNextFileName(directory+os.sep+filename+'_ch%02d'%channel , "gsas")
        ...
        # find which calibration corresponds to the channel
        calib_ndx = np.nonzero(self.multicalib_chan==channel)[0][0]
        calib = self.multicalib_E[calib_ndx, :]
        ...
        for ii in range(int(nrec)):
            ...
            for jj in range(lrec):
                ndx = (ii*lrec)+jj
                if ndx<nchan:
                    mynum = "{:d},".format(int(self.spectra[calib_ndx, ndx])).rjust(8)
                    myerr = "{:d},".format(int(np.sqrt(self.spectra[calib_ndx, ndx]))).rjust(8)
        ...
        # write a text file for this channel as well
        ...
        for ii in range(nchan):
            myfile.write("{:.3f}".format(self.energy[calib_ndx, ii]) + " " + "{:d}".format(int(self.spectra[calib_ndx, ii])) + "\r\n")
        myfile.close()
        ...
        #myfile.write(filename + "\r\n")
        for ii in range(nchan):
            myfile.write("{:d}".format(int(self.energy[calib_ndx, ii]*1000)) + " " + "{:d}".format(int(self.spectra[calib_ndx, ii])) + "\r\n")
        myfile.close()
        print("pd txt file: \n  %s\nwritten" % pdtextfilename)

    def fastsave(self, filename="psiche", directory="/nfs/ruche-psiche/psiche-soleil/tempdata/com-psiche/test_directory"):
        '''Save two files - one with the raw data, one with the dspacing data'''
        # get the current spectrum !
        self.updateSpectrum()
        # save it
        myfilename = findNextFileName(directory+os.sep+filename+'_raw' , "txt")
        myfile = open(myfilename, 'wt')
        myfile.write('# %s\n' % time.localtime())
        myfile.write('# channel ch00 ch01 ch02 ch03 ch04 ch05 ch06\n')
        for ii in range(self.spectra.shape[1]):
            myfile.write("%d %d %d %d %d %d %d %d\n" % (ii, self.spectra[0, ii], self.spectra[1, ii], self.spectra[2, ii], self.spectra[3, ii], self.spectra[4, ii], self.spectra[5, ii], self.spectra[6, ii]))
        myfile.close()
        print('%s written and closed' % myfilename)
        myfilename = findNextFileName(directory+os.sep+filename+'_dsp' , "txt")
        myfile = open(myfilename, 'wt')
        myfile.write('# %s\n' % time.localtime())
        myfile.write('# dsp ch00 ch01 ch02 ch03 ch04 ch05 ch06\n')
        for ii in range(len(self.spectra_dspx)):
            myfile.write("%f %f %f %f %f %f %f %f\n" % (self.spectra_dspx[ii], self.spectra_dspy[0, ii], self.spectra_dspy[1, ii], self.spectra_dspy[2, ii], self.spectra_dspy[3, ii], self.spectra_dspy[4, ii], self.spectra_dspy[5, ii], self.spectra_dspy[6, ii]))
        myfile.close()
        print('%s written and closed' % myfilename)
    ...
    def __exportGSAS_combined(self):
        '''export the single profile summary of a Caesar scan as a pseudo ADX spectra at 35 keV
            for multi channel - export each channel, plus a combined one'''
        if self.caesar_image_dsp!="":
            if len(self.caesar_image_dsp_profile.shape)==1: # single channnel case
            ...
            elif len(self.caesar_image_dsp_profile.shape)==2: # multi channnel case
                print("Exporting single ADX profile summarising Caesar acquistion - each channel and average! ")
                pQ = self.caesar_image_Qx*1. # same x scale for all - use Q rather than d
                # "scan name" is the name of the directory
                scanname = self.datadir.split(os.sep)[-1]
                for chan in range(self.nchan + 1): # do all channels, plus 1 for average
                    if chan<self.nchan:
                        filenamegsas = self.datadir + os.sep + scanname + "_combined_35keV_ch%02d" % chan
                        filenametxtQ = self.datadir + os.sep + scanname + "_Q_ch%02d" % chan
                        filenametxtQcor = self.datadir + os.sep + scanname + "_Q_cor_ch%02d" % chan
                        filenametxtd = self.datadir + os.sep + scanname + "_dsp_ch%02d" % chan
                        filenametxtdcor = self.datadir + os.sep + scanname + "_dsp_cor_ch%02d" % chan
                        # convert to ADX profile
                        pint = self.caesar_image_Q_profile[chan, :]*1.# - use Q rather than d
                    else:
                        # make average and special names
                        pint = self.caesar_image_Q_profile.mean(0)# - use Q rather than d
                        filenamegsas = self.datadir + os.sep + scanname + "_combined_35keV_chAll"
                        filenametxtQ = self.datadir + os.sep + scanname + "_Q_chALL"
                        filenametxtQcor = self.datadir + os.sep + scanname + "_Q_cor_chAll"
                        filenametxtd = self.datadir + os.sep + scanname + "_dsp_chAll"
                        filenametxtdcor = self.datadir + os.sep + scanname + "_dsp_cor_chAll"

                    # convert and write files
                    wlength = 12.398/35
                    pdsp = old_div(2*np.pi, pQ)
                    ptth = 2*np.arcsin(old_div(wlength,(2*pdsp)))
                    # interpolate to constant steps in tth
                    ptthgrid = np.linspace(ptth[0], ptth[-1], 4096)
                    pintgrid = np.interp(ptthgrid, ptth, pint, 0, 0)
                    ...
                    if pintgrid.max() > 99999:
                        ...
                        pintgrid = old_div(pintgrid, (10**fac))
                    # write this as a GSAS file
                    myfilename = findNextFileName(filenamegsas, "gsas")
                    # open the file
                    myfile = open(myfilename, 'w')
                    # write the title
                    myline = myfilename.split(os.sep)[-1] + " (combined Caesar scan)"
                    ...
                    # export two column text file too - Q
                    if self.caesar_image_Q_profile != '':
                        myfilenametxt = findNextFileName(filenametxtQ, "txt")
                        myfiletxt = open(myfilenametxt, 'wt')
                        myfiletxt.write('Q(A-1), intensity\n')
                        if chan<self.nchan:
                            profile = self.caesar_image_Q_profile[chan, :]
                        else:
                            profile = self.caesar_image_Q_profile.mean(0)
                        for ii in range(len(self.caesar_image_Qx)):
                            myfiletxt.write('%f,%f\n' % (self.caesar_image_Qx[ii], profile[ii]))
                        myfiletxt.close()
                        print("text file %s written and closed..." % myfilenametxt)
                    # export two column text file too - dsp
                    if self.caesar_image_dsp_profile != '':
                        myfilenametxt = findNextFileName(filenametxtd, "txt")
                        myfiletxt = open(myfilenametxt, 'wt')
                        myfiletxt.write('d(A), intensity\n')
                        if chan<self.nchan:
                            profile = self.caesar_image_dsp_profile[chan, :]
                        else:
                            profile = self.caesar_image_dsp_profile.mean(0)
                        for ii in range(len(self.caesar_image_dspx)):
                            myfiletxt.write('%0.8f,%0.8f\n' % (self.caesar_image_dspx[ii], profile[ii]))
                        myfiletxt.close()
                        print("text file %s written and closed..." % myfilenametxt)
                    # export two column text file too - Q cor - smoothed
                    if self.caesar_image_Q_profile_cor != '':
                        myfilenametxt = findNextFileName(filenametxtQcor, "txt")
                        myfiletxt = open(myfilenametxt, 'wt')
                        myfiletxt.write('Q(A-1), intensity\n')
                        if chan<self.nchan:
                            profile = self.caesar_image_Q_profile_cor[chan, :]
                        else:
                            profile = self.caesar_image_Q_profile_cor.mean(0)
                        for ii in range(len(self.caesar_image_Qx)):
                            myfiletxt.write('%f,%f\n' % (self.caesar_image_Qx[ii], profile[ii]))
                        myfiletxt.close()
                        print("text file %s written and closed..." % myfilenametxt)
                    # export two column text file too - dsp cor - smoothed
                    if self.caesar_image_dsp_profile_cor != '':
                        myfilenametxt = findNextFileName(filenametxtdcor, "txt")
                        myfiletxt = open(myfilenametxt, 'wt')
                        myfiletxt.write('d(A), intensity\n')
                        if chan<self.nchan:
                            profile = self.caesar_image_dsp_profile_cor[chan, :]
                        else:
                            profile = self.caesar_image_dsp_profile_cor.mean(0)
                        for ii in range(len(self.caesar_image_dspx)):
                            myfiletxt.write('%0.8f,%0.8f\n' % (self.caesar_image_dspx[ii], profile[ii]))
                        myfiletxt.close()
                        print("text file %s written and closed..." % myfilenametxt)
